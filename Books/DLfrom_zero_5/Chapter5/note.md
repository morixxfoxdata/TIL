...
実際は $\theta = \theta_{\text{old}}$ で固定しているので $q(z) = p_{\theta_{\text{old}}}(z|x)$ が更新式になる.

これは EM アルゴリズムの E ステップと呼ばれる. これは $q(z) = p_{\theta_{\text{old}}}(z|x)$ であるなら ELBO が期待値となるから.

E ステップでは $q(z)$ を更新して ELBO を $\log p(x;\theta)$ に近づける(イコールにする)作業を行う.
次に $\theta$ の最適化を行う. ELBO が"sum-log"の形をしているため解析的に求められる.
ELBO の式を $\theta$ について微分して 0 の点の $\theta = \theta_{\text{new}}$ として更新する. これを M ステップという.

これを更新し続けることで対数尤度 $\log p_{\theta}(x)$ を徐々に大きくすることができる.

EM アルゴリズムの性質として次の式が常に成り立つ.

$$
\log p(x;\theta_{\text{new}}) \geq \log p(x;\theta_{\text{old}})
$$

つまり一回の更新で対数尤度は常に単調に増加する.(後で証明)

### 5.3.3 複数データへの拡張

N 個の観測データ $\lbrace{x^{(1)}, x^{(2)}, \dots , x^{(N)}}\rbrace$ に対して任意の確率分布 $\{q^{(1)}, q^{(2)}, \dots, q^{(N)}\}$ を用意する.

以上から EM アルゴリズムは以下のようにまとめられる
